// 📁 src/lava/googlemap/converter.ts - Google Maps 기준으로 리팩토링된 converter.ts

import { defaultZoom } from './controller';
import { ISize, IPoint, Func, StringMap, clamp } from '../type';
import { pixel } from './controller'; // ✅ controller에서 google.maps.Map 기반 pixel 계산 사용

export function anchorPixel(map: google.maps.Map, bound: IBound): IPoint {
    return pixel(map, bound.anchor);
}
